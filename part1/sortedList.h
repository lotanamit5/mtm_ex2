...
public:
    ...
    bool insert(const T element);                         //maybe bool or exception
    bool remove(const SortedList<T>::const_iterator &it); //maybe bool or exception, need iterator
    int length();
...
class SortedList<T>::const_iterator
{
    ...
    /*{
        if (node_new != nullptr)
        {
            node = new Node<T>(node_new->data);
            node->next = node_new->next;
            node->previous = node_new->previous;
        }
    }*/
